...
const static unsigned int note2msec[] = {BASE/4, BASE/2, BASE, BASE*2, BASE*4};
...
void buzzer_beep(int freq, int msec)
{
	...
	//iowrite32()
	//1. PWM_CTL -> 0x80 --stop buzzer

	iowrite32(0x80,(void *)(pwm_base + PWM_CTL));	
	//2. PWN_RNG1, PWM_DAT1 set by frequency
	iowrite32(clk_rate/freq, (void *)(pwm_base+PWM_RNG1));
	iowrite32(clk_rate/freq/2, (void *)(pwm_base+PWM_DAT1));
	//3. PWN_CTL -> 0x81 --buzzer
	iowrite32(0x81, (void *)(pwm_base+PWM_CTL));
	//4. msleep(msec); --uninterruptible sleep
	msleep_interruptible(msec);
	//5. PWN_CTL 0 -> 0x80 --stop
	iowrite32(0x80, (void *)(pwm_base + PWM_CTL));
	//msleep interruptible - ctrl+c
}
