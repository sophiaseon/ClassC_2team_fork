...
{
	printk("devtest: device_mmap\n");
	printk("vm_start=%#lx, vm_end=%#lx, vm_page_prot=%#llx, vm_pgoff=%#lx\n",
			vma->vm_start, vma->vm_end, vma->vm_page_prot.pgprot, vma->vm_pgoff);
	if (remap_pfn_range(vma,
				vma->vm_start,
				virt_to_phys(write_buf) >> PAGE_SHIFT,
				vma->vm_end - vma->vm_start,
				vma->vm_page_prot)) {
		return -EAGAIN;
	}
	strcpy(write_buf, "Hello Linux");
	return 0;
}
#endif
...
#if TEST_MMAP
	write_buf = (char *)__get_free_page(GFP_KERNEL);
	if(write_buf == NULL) {
		printk("devtest: failed to get virtual address\n");
		return -ENOMEM;
	}
	printk("devtest: write_buf allocated at %p\n", write_buf);
#endif
...
#if TEST_MMAP
	free_page((unsigned long)write_buf);
#endif /* TEST_MMAP */
