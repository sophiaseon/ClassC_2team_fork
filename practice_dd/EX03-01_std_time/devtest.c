...
static int __init device_init(void)
{
...
#if TEST_TIME
	{
		...
		printk("devtest: HZ=%d, jiffies=0x%lx\n", HZ, jiffies);

		printk("devtest: delay 1 sec with busy waiting\n");
		t1 = jiffies + HZ;
		while(time_before(jiffies, t1)) cpu_relax();

		printk("devtest: delay 1 sec with yielding\n");
		t1 = jiffies + HZ;
		while(time_before(jiffies, t1)) schedule();

		printk("devtest: delay 1 sec with wait_event_timeout\n");
		init_waitqueue_head(&wait);
		wait_event_timeout(wait, 0, HZ);

		printk("devtest: delay 1 sec with wait_event_interruptible_timeout\n");
		init_waitqueue_head(&wait);
		wait_event_interruptible_timeout(wait, 0, HZ);

		printk("devtest: delay 1 sec with schedule_timeout() (INTERRUPTIBLE)\n");
		set_current_state(TASK_INTERRUPTIBLE);
		schedule_timeout(HZ);

		printk("devtest: delay 1 sec with schedule_timeout() (UNINTERRUPTIBLE)\n");
		set_current_state(TASK_UNINTERRUPTIBLE);
		schedule_timeout(HZ);

		printk("devtest: delay 1 sec with mdelay\n");
		mdelay(1000);

		printk("devtest: delay 1 sec with msleep\n");
		msleep(1000);
	}
#endif
