...
#define CONF_DT_GPIO 1

#if CONF_DT_GPIO
#include <linux/gpio/consumer.h>
static struct gpio_desc *led2, *led3;
#else
static int phy_base;
static int phy_size;
#endif
...
static int driver_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(led2)) {
		printk("devtest: can't get gpiod\n");
		goto err2;
	}
	led3 = gpiod_get_index(&pdev->dev, "led", 1, GPIOD_OUT_LOW);
	if (IS_ERR(led3)) {
		printk("devtest: can't get gpiod\n");
		gpiod_put(led2);
		goto err2;
